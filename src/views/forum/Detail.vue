...
import { getPostDetail, likePost, createComment, getComments, createReport, deletePost } from '@/mock/api'
...
async function onSubmitComment() {
  ...
  try {
    const res = await createComment({ postId: postId.value, content: text, isAnonymous: false })
    if (res.code === 0) {
...
async function onReport() {
  const confirm = await showModal('举报', '确认举报该帖子？')
  if (!confirm) return
  try {
    const res = await createReport({ targetType: 'post', targetId: postId.value, reason: '内容违规' })
    if (res.code === 0) showToast('举报成功', 'success')
  } catch (e) {
    ...
    <div v-else-if="post">
      <div class="post-section">
        <div class="post-header">
          <div class="avatar">{{ post.isAnonymous ? '匿' : (post.authorNickName ? post.authorNickName[0] : '用') }}</div>
          <div class="author-info">
            <div class="name">{{ post.isAnonymous ? '匿名用户' : post.authorNickName }}</div>
            <div class="meta">{{ post.timeStr }} {{ post.category ? '· ' + post.category : '' }}</div>
          </div>
        </div>
        ...
        <div class="post-actions">
          ...
          <div class="action-item" @click="onReport">
            <span class="icon">🚩</span>
            <span>举报</span>
          </div>
